...
3. **生成分镜脚本**
   - 根据大纲 + 项目上下文生成详细分镜
   - 每个场景包含：
     - **画面描述**：人物表情、动作、环境细节、镜头角度（远景/中景/特写）、光线氛围
...
2. **抓取热点**
   - 使用 `web_search` 或 `browser_navigate` 抓取：
     - B站热门视频（bilibili.com/ranking）
     - 抖音热榜（不可直接抓取，通过搜索引擎查询"抖音热榜"）
     - 微博热搜（s.weibo.com/top/summary）
     - 知乎热榜（zhihu.com/hot）
     - 百度热搜（top.baidu.com/board）
     - 豆瓣高分（douban.com）
   - 时效性：15天内
   - 类型：全类型
...
## Scene 4
**镜头**：双人中景  
**画面**：两人面对面站立，夕阳光晕笼罩两人。小雨双手背在身后，微微低头，害羞地笑着。阿明挠了挠头，有些不自在。  
**对话**：  
- 小雨："我有话想跟你说……"  
- 阿明："什么？"
