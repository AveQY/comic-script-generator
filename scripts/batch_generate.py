...
def get_hermes_api_config() -> dict:
    ...
    if not config_path:
        ...
        return {
            "base_url": os.environ.get("HERMES_BASE_URL", "https://cli-api.YOUR_DOMAIN/v1").rstrip("/"),
...
def generate_outline(topic: str, project_name: str, mode: str,
    ...
    base_url = config.get("base_url", "https://cli-api.YOUR_DOMAIN/v1").rstrip("/")
    ...
    mode_desc = {
        "A": "对话多，镜头少（传统漫画风，每集30-40个Panel，每场景1-2个镜头+3-8段对话）",
        "B": "一个对话对应一个镜头（平衡模式，每集50-60个Panel）",
        "C": "一段对话多个镜头（电影短剧风，每集150-250个Panel，画面细节丰富）",
    }
...
def generate_episode(ep_num: int, outline: str, mode: str, project_name: str) -> str:
    ...
    mode_desc = {
        "A": "对话多，镜头少（传统漫画风，每集30-40个Panel）",
        "B": "一个对话对应一个镜头（平衡模式，每集50-60个Panel）",
        "C": "一段对话多个镜头（电影短剧风，每集150-250个Panel）",
    }
    ...
    ep_prompt = (
        ...
        "【要求】\n"
        "- 分镜密度：{mode_desc}\n"
        "- 每个 Panel：格子、画面、构图、气泡、旁白、拟声、转场、AI提示词（正向+反向）\n"
        "- 生成稿子时必须一次性符合正常漫画逻辑，禁止事后才靠修复器补救。\n"
        "- **画面字段必须是可直接绘制的具体画面**：写清角色姓名/外貌、物件、动作前后顺序、地点、光线、情绪和画面焦点。\n"
        "- 禁止抽象模板句：不得写“普通日常场景”“异常事件突然出现”“主角靠近异常源”“关键配角登场”“世界观第一次展开”“危机升级”“情感冲突爆发”“关键线索出现”等空泛描述。\n"
        "- 每个 Panel 都要承接上一格：动作、视线、物件位置或情绪至少一个连续；不能 5 格重复同一构图。\n"
        "- 图片生成阶段不负责生成中文文字：AI提示词必须包含 no text/no letters/leave empty space for speech bubbles；对白、拟声只写在气泡/拟声字段，后续由 overlay 脚本叠加。\n"
        "- 少旁白，优先用角色对白气泡和拟声推进；没有旁白/拟声也必须写“无”。\n"
        ...
        "## Page 1\n\n"
        "### Panel 1\n\n"
        "**格子**：[普通格/横向大格/竖向窄格/半页大格/跨页大格]\n"
        "**画面**：[具体可画画面：角色姓名+动作+表情+关键物件+地点+光线；例：陈野穿黄色雨衣蹲在发光外卖箱前，手停在拉链上，雨水从帽檐滴下]\n"
        "**构图**：[具体镜头：远景/中景/特写/俯拍/仰拍/过肩/双人构图 + 视觉焦点 + 气泡留白位置]\n\n"
        "**气泡**：\n- [角色]（[位置]，[气泡类型]）：[台词]\n\n"
        "**旁白**：无\n\n"
        "**拟声**：无\n\n"
